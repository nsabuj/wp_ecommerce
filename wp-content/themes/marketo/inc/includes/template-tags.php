...
if ( !function_exists( 'marketo_post_meta' ) ) {
	...
	function marketo_post_meta() {
		...
		if ( get_post_type() === 'post' ) {
			...
			if ( is_sticky() ) {
				echo '<span class="meta-featured-post sticky"> <i class="fa fa-thumb-tack"></i> ' . esc_html__( 'Featured', 'marketo' ) . ' </span>';
			}
			if ( is_single() ) {
				// Get the post author.

				printf(
				'<span class="meta-author post-author">%1$s <a href="%2$s" rel="author">  ' . esc_html__( 'By ', 'marketo' ) . '  %3$s</a></span>', get_avatar( get_the_author_meta( 'ID' ), 55 ), esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), get_the_author()
				);
			}
			// The categories.
